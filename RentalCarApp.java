...
public class RentalCarApp {
    ...
    JButton adminLoginButton = new JButton("Admin Login"); // New Admin Login Button
    JPanel buttonPanel = new JPanel();
    buttonPanel.add(loginButton);
    buttonPanel.add(signupButton);
    buttonPanel.add(adminLoginButton); // Add Admin Login Button to Panel
    ...
    adminLoginButton.addActionListener(e -> {
        String adminPassword = JOptionPane.showInputDialog(authFrame, "Enter Admin Password:");
        if ("admin123".equals(adminPassword)) { // Replace with a secure authentication mechanism
            authFrame.dispose();
            new AdminDashboard(); // Navigate to the Admin Dashboard
        } else {
            JOptionPane.showMessageDialog(authFrame, "Invalid Admin Password.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    });
